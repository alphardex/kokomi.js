...
float snoise(vec3 v)
{
    ...
    vec4 p=permute(permute(permute(
            ...
            void main(){
                ...
                vec4 texture1=texture(uTexture1,p);
                vec4 texture2=texture(uTexture2,p);
                vec3 colorA=texture1.rgb;
                vec3 colorB=texture2.rgb;
                
                vec2 cUv=centerUv(vUv,iResolution);
                vec2 cHoverUv=centerUv(uHoverUv/uDevicePixelRatio,iResolution);
                
                // circle
                float circleShape=1.-distance(cUv,cHoverUv);
                
                // noise
                float uvX=cUv.x+sin(cUv.y+iTime*.1);
                float uvY=cUv.y-iTime*.1;
                vec3 distortedUv=vec3(uvX,uvY,iTime*.1)*8.;
